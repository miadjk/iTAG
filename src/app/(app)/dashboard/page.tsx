  ...
  return (
    <div>
      ...
      {user.mustUpdateCredentials ? (
        <div className="mb-6 flex items-start gap-3 border border-[#F1E5A1] p-4">
          <AlertCircle className="h-5 w-5 text-[#F1E5A1]" />
          <div>
            <p className="text-sm">Update your School Head profile before operating the school inventory.</p>
            <Link href="/profile?setup=1" className="mt-2 inline-flex items-center gap-2 text-xs uppercase tracking-widest text-[#8a7310] dark:text-[#F1E5A1]">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section className="surface p-5">
          ...
          <div className="space-y-3 text-sm">
            {can("encode") ? (
              <Link href="/properties/new" className="flex items-center justify-between border border-[var(--border)] p-3 text-[var(--text)] hover:border-[#F1E5A1]">
                Add a property <ArrowRight className="h-4 w-4 text-[#F1E5A1]" />
              </Link>
            ) : (
              <Link href="/properties" className="flex items-center justify-between border border-[var(--border)] p-3 text-[var(--text)] hover:border-[#F1E5A1]">
                Review property records <ArrowRight className="h-4 w-4 text-[#F1E5A1]" />
              </Link>
            )}
            <Link href="/assignments" className="flex items-center justify-between border border-[var(--border)] p-3 text-[var(--text)] hover:border-[#F1E5A1]">
              Assignments <ArrowRight className="h-4 w-4 text-[#F1E5A1]" />
            </Link>
            <Link href="/audit" className="flex items-center justify-between border border-[var(--border)] p-3 text-[var(--text)] hover:border-[#F1E5A1]">
              Activity <ArrowRight className="h-4 w-4 text-[#F1E5A1]" />
            </Link>
          </div>
        </section>
        ...
        <section className="surface p-5">
          ...
          <div className="mt-4 space-y-3">
            {unreadNotifications.slice(0, 5).map((n) => (
              <div key={n.id} className="border border-[var(--border)] p-3">
                <p className="text-sm">{n.title}</p>
                <p className="mt-1 text-xs text-[var(--text-muted)]">{n.body}</p>
              </div>
            ))}
      ...
      <section className="surface mt-6 p-5">
        <h2 className="font-display text-2xl">Recent property activity</h2>
        <div className="mt-4 space-y-3">
      ...
      {schoolSupplies.some((s) => s.status !== "available") ? (
        <section className="mt-6">
          <h2 className="font-display mb-3 text-2xl">Supply attention</h2>
...
function Stat({ label, value, icon: Icon }: { label: string; value: number; icon: typeof Package }) {
  return (
    <div className="surface p-5">
      <Icon className="h-4 w-4 text-[#F1E5A1]" />
